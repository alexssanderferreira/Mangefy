...
| `tabs.apply_discount`          | Aplicar desconto na comanda até o limite de `TabSettings.MaxDiscountPercent` |
| `tabs.apply_discount_override` | Aplicar desconto acima do limite configurado                |
| `tabs.apply_courtesy`          | Registrar cortesia (comanda sem cobrança)                   |
...
| `orders.cancel`                | Cancelar itens no status Pending                            |
| `orders.cancel_after_sent`     | Cancelar itens já enviados à cozinha (Sent / Preparing)     |
| `orders.cancel_in_preparation` | Cancelar itens em preparo (Preparing / Ready)               |
| `orders.cancel_delivered`      | Cancelar itens já entregues ao cliente                      |
