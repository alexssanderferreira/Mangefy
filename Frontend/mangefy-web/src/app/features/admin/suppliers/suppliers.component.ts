...
@Component({
    ...
    @if (drawer()) {
      ...
      <aside class="drawer">
        ...
        <form class="drawer-body" [formGroup]="supplierForm" (ngSubmit)="saveSupplier()">
          <div class="field">
            <label>Nome *</label>
            <input formControlName="name" placeholder="Nome do fornecedor" />
          </div>
          <div class="field">
            <label>Categoria *</label>
            <select formControlName="supplierCategoryId">
          ...
          <div class="field-row">
            <div class="field">
              <label>CNPJ</label>
              <input formControlName="cnpj" placeholder="00.000.000/0001-00" />
            </div>
            <div class="field">
              <label>Telefone</label>
              <input formControlName="phone" placeholder="+55 11 9..." />
            </div>
          </div>
          <div class="field">
            <label>E-mail</label>
            <input formControlName="email" type="email" placeholder="contato@fornecedor.com" />
          </div>
          <div class="field">
            <label>Website</label>
          ...
          <div class="field">
            ...
            <textarea formControlName="description" rows="3" placeholder="Descrição opcional..."></textarea>
          </div>
    ...
    @if (catDrawer()) {
      ...
      <aside class="drawer">
        ...
        <form class="drawer-body" [formGroup]="catForm" (ngSubmit)="saveCategory()">
          <div class="field">
            <label>Nome *</label>
            <input formControlName="name" placeholder="Ex: Laticínios, Bebidas..." />
          </div>
          <div class="field">
            <label>Descrição</label>
            <textarea formControlName="description" rows="3" placeholder="Descrição opcional..."></textarea>
  ...
  styles: [`
    ...
    /* Drawer */
    .overlay { position: fixed; inset: 0; background: rgba(0,0,0,.3); z-index: 40; }
    .drawer {
    ...
    .drawer-footer { display: flex; gap: 8px; justify-content: flex-end; padding-top: 8px; margin-top: auto; }

    .field { display: flex; flex-direction: column; gap: 5px; }
    .field-row { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; }
    .field label { font-size: 12px; font-weight: 600; color: #444; }
    .field input, .field select, .field textarea {
      padding: 9px 12px; border: 1.5px solid #e8e8ec; border-radius: 8px;
...
export class SuppliersComponent implements OnInit {
  ...
  openEdit(s: PlatformSupplierDto) {
    ...
    this.supplierForm.patchValue({ name: s.name, supplierCategoryId: s.supplierCategoryId, cnpj: s.cnpj ?? '', phone: s.phone ?? '', email: s.email ?? '', website: s.website ?? '', description: s.description ?? '' });
    this.drawer.set(true);
  }
  ...
  saveSupplier() {
    ...
    const body = { name: v.name!, supplierCategoryId: v.supplierCategoryId!, cnpj: v.cnpj || null, phone: v.phone || null, email: v.email || null, website: v.website || null, description: v.description || null };
