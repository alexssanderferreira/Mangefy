...
public sealed class UpdateMenuCategoryCommandHandler : IRequestHandler<UpdateMenuCategoryCommand>
{
    private readonly IMenuRepository _menus;
    private readonly IUnitOfWork _uow;

    public UpdateMenuCategoryCommandHandler(IMenuRepository menus, IUnitOfWork uow)
    {
        _menus = menus;
        _uow = uow;
    }
    ...
    public async Task Handle(UpdateMenuCategoryCommand request, CancellationToken cancellationToken)
    {
        ...
        menu.UpdateCategory(request.CategoryId, request.Name, request.Description, request.DisplayOrder);
